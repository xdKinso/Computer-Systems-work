...
static ssize_t device_write(struct file *file, const char *buf, size_t len, loff_t *offset)
{
    int result;
    char kernel_buffer[255];
    // Copy data from user memory, storing result
    // *** COMPLETE ME ***
    result = copy_from_user(kernel_buffer, buf, len);

    if (result) {
        printk(KERN_ERR "Device write error\n");
    }
    printk(KERN_INFO "Device received %s", kernel_buffer);
    return len;
}
