...
int main(int argc, char **argv)
{
    ...
    for (int i = 0; i < NUM_THREADS; ++i)
    {
        ...
        // Set that we want to have an explicit scheduler
        // *** COMPLETE ME ***
        pthread_attr_setinheritsched(&attr, PTHREAD_EXPLICIT_SCHED);
        // Set scheduler policy to SCHED_RR
        // *** COMPLETE ME ***
        pthread_attr_setschedpolicy(&attr, SCHED_RR);
        // Create the thread - remember to pass a pointer to attr
        // *** COMPLETE ME ***
        pthread_create(&threads[i], &attr, do_work, i);
    }
    void *status;
    for (int i = 0; i < NUM_THREADS; ++i)
    {
